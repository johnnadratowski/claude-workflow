---
name: define-project
description: Interactive, multi-stage project definition flow. Drives a deep dialog with the user across product → architect → QA → deploy/security → tracking-taxonomy, populating `docs/` with detailed specs, scaffolding code where applicable, and tailoring the built-in TODO system's taxonomy at the end. Each stage is a dedicated subskill (`define-product`, `define-architect`, `define-qa`, `define-deploy`, `define-tickets`) with subagent-driven critical review and user signoff gates. On re-entry (after the codebase is initialized) it asks which stage to update and re-runs only that subskill in update mode.
---
...
| 5 | `define-tickets` | The work-tracking taxonomy for the built-in TODO system | `docs/todos/milestones.json` (areas → ID prefixes, priorities, milestones), TODO conventions in `docs/todos/README.md`. No external provider; the TODO files are the tracker. |
...
- **Tracking taxonomy** — TODO areas / priorities / milestones / conventions (`define-tickets`)
...
- `base-initialize` — calls this skill on first project setup (after the repo reset, before worktree creation).
- `define-product`, `define-architect`, `define-qa`, `define-deploy`, `define-tickets` — the five subskills.
- `todo` — the day-to-day workflow that runs *against* the specs + TODO taxonomy this flow produces; the TODO files under `docs/todos/` are the tracker.
