...
- Coordination is **purely local**: local `<base>` is the source of truth; only `/base-push` touches origin. Use `agent-send --stdin` (never an argv body — it corrupts on shell metacharacters) and `--followup` for a reply that needs an answer.
