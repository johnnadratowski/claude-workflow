...
# Self-heal: hooks aren't always reliable across `claude --resume` paths
# in every Claude Code version, so the registry can drift. Run the
# idempotent registration script first (fast-path no-op if the entry
# is already current) so a stale or missing own-entry is repaired
...
# --- Deliver ---
slash="/agent-msg $self_name $msg_id.txt"
