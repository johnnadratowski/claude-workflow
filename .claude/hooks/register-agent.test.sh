...
# --- Harness ---------------------------------------------------------------
newhome(){ local h; h="$(mktemp -d)"; mkdir -p "$h/.claude/sessions" "$h/.claude/running-agents" "$h/.claude/agents"; printf '%s' "$h"; }
...
# exports WORKFLOW_AGENT_SKIP_RENAME=1, which would silently suppress every
...
echo "== 9. still-transient keystroke: startup types, SKIP_RENAME suppresses =="
H="$(newhome)"; R="$(newrepo)"; CB="$(sanitize "$(basename "$R")")"
...
ok "SKIP_RENAME=1 suppresses the keystroke" '! grep -q "/rename" "$KLOG"'
rm -f "$KLOG"; rm -rf "$H" "$R"
