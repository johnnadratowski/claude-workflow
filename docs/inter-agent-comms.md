...
8. **Waking parked agents.** The drain covers any agent that takes another turn, but a fully idle agent has already Stopped and won't run its drain until woken. `.claude/scripts/inbox-watcher.sh` is a poller — run continuously by the coordinator (kept alive by the `cc-watcher-keepalive.sh` hook) — that re-nudges live recipients with the reconstructed `/agent-msg` for any message left sitting past a timeout, closing that last gap.
...
| `.claude/scripts/inbox-watcher.sh` | The **coordinator-run** poller that re-nudges parked (already-Stopped) agents AND retriable-errored agents (rate-limit / overloaded / server-error StopFailure marker) to continue — the idle-fleet gap the drain hook cannot cover. The cc keeps exactly one instance alive via the `cc-watcher-keepalive.sh` hook (SessionStart launch + Stop keep-alive). Pure send-keys, no model calls. |
